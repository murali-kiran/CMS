...
<c:forEach var="categoryMap" items="${mediaInfoMap}">
	...
	<c:when test="${fn:length(media.name) eq 3}">
	   ...
	   <c:choose>
	    <c:when test="${mediaInfo.isSubMediaGroup eq true}">
  		...
  		<a href="${pageContext.servletContext.contextPath}/stv/cat/${serviceId}/${mediaInfo.mediagroupParentId}/${mediaInfo.mediagroupId}?channel=${channel}">
  		...
  		<a href="<c:url value="/stv/dwl/${serviceId}/${mediaInfo.mediaId}/${mediaInfo.serviceKeypriceKey}?channel=${channel}&serviceKeyId=${mediaInfo.serviceKeyId}&catId=${category.id}"/>">
		<img alt="noImage" src="${mediaInfo.storagePath}"
					width="${previewWidth}" height="${previewHeight}">
		</a>			
		</div>
	...
	&nbsp;<a href="${pageContext.servletContext.contextPath}/stv/cat/${serviceId}/${category.id}?channel=${channel}&serviceKeyId=${serkeyid}">More</a>&nbsp;
	</div>
	</c:when>
	<c:otherwise>
