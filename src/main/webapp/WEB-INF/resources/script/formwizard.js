	...
	init:function(setting){
		...
		jQuery(function($){ //on document.ready
			...
			//	var $thestep=$('<div class="step disabledstep" />').data('section', i).html('Step '+(i+1)+'<div class="smalltext">'+$section.find('legend:eq(0)').text()+'</div>').appendTo($stepsguide)
				var $thestep=$('<div class="step disabledstep" />').data('section', i).html('<div class="smalltext">'+$section.find('legend:eq(0)').text()+'</div>').appendTo($stepsguide)
			...
			var $paginatediv=$('<div class="formpaginate" style="overflow:hidden;"> </div>')
