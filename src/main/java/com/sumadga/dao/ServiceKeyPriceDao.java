...
public class ServiceKeyPriceDao {
	...
	@SuppressWarnings("unchecked")
	public List<ServiceKeyPrice> findByServiceKey(int serviceKeyId) {
		logger.info("finding all ServiceKeyPrice instances");
		try {
			final String queryString = "select model from ServiceKeyPrice model where model.serviceKey = "+serviceKeyId;
			Query query = entityManager
					.createQuery(queryString, ServiceKeyPrice.class);
