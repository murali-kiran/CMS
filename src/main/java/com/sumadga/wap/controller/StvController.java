...
@Controller
@Scope("request")
public class StvController extends BaseController{
	
	Logger logger = Logger.getLogger(StvController.class);
	...
	@RequestMapping(value="/stv/{serviceId}",method=RequestMethod.GET)
	...
	//	String msisdn = commonUtils.getMsisdn(request);
		...
		return "stvLandingPage";
		
	}
	
	
	@RequestMapping(value="/stv/dwlFile/{serviceId}/{mediaId}/{purchaseId}",method=RequestMethod.GET)
...
@RequestMapping(value="/stv/dwl/{serviceId}/{mediaId}/{serviceKeypriceKey}",method=RequestMethod.GET)	
public String downloadMedia(HttpServletRequest request,HttpServletResponse response,HttpSession session,Model model,@PathVariable Integer serviceId,@PathVariable Integer mediaId,@PathVariable String serviceKeypriceKey,@RequestParam(value = "channel", required = false,defaultValue="smd") String channel,
		...
		/*if(!isTestMobileNumber && request.getParameter("responsecode") == null ){
			...
			String mediaIdsStr = serviceLayer.getAllMediaIdInStringFormat(catId);
			
			System.out.println(mediaIdsStr);
			...
			logger.info("msisdn:"+msisdn+" identifier:"+identifier+ " operator :");
				...
				Purchas purchase =	serviceLayer.checkPurchaseRecordOfMediaGroupExistForToday(msisdnLong,catId, identifier);
				...
				if(isTestMobileNumber || ( responseCode!=null && responseCode.equals("101") ) || purchase != null){
					...
					model.addAttribute("isDownloadURL", true);
					model.addAttribute("downloadURL", request.getContextPath()+"/stv/dwlFile/"+serviceId+"/"+mediaId+"/"+purchase.getPurchaseId()+"?channel="+channel);
					return getService(model, serviceId, request, channel);
				}else{
				...
				return "forward:/stv/"+serviceId+"?channel="+channel;
				}
				else{
					logger.info("unable to detect MSISDN");
					model.addAttribute("message", "Unable to detect MSISDN");
					return "forward:/stv/"+serviceId+"?channel="+channel.toString();
				}
				
	}

	
	@RequestMapping(value="/stv/cat/{serviceId}/{catId}",method=RequestMethod.GET)
	public String getServiceByCategory(HttpServletRequest request,Model model,@PathVariable Integer serviceId,@PathVariable Integer catId,@RequestParam(value = "channel", required = false,defaultValue="smd") String channel){
		...
		return "stvService2CategoryPage";
	}
	
	@RequestMapping(value="/stv/cat/{serviceId}/{parentCatId}/{catId}",method=RequestMethod.GET)
	...
	@RequestMapping(value="/stv/cat/pageId/pageCount/{serviceId}/{catId}/{pageId}/{pageCount}",method=RequestMethod.GET)
	...
	@RequestMapping(value="/stv/showSearchPage/{serviceId}",method=RequestMethod.GET)
...
//		model.addAttribute("title", "Search");
		model.addAttribute("channel", channel);
		return "stvSearchByTag";
	}
	
	@RequestMapping(value="/stv/searchMediaByTag/{serviceId}/{pageId}",method=RequestMethod.GET)
...
//		model.addAttribute("title", "Search");
		...
		if(StringUtils.isBlank(tag)){
			...
			return "stvTagMediaPage";
		}
	}
	
	@RequestMapping(value="main/stv/service/ipayBillingResponse",method=RequestMethod.GET)
